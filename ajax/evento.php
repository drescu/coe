...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        ...
        if (empty($idpersona)) {  
            $rspta = $evento->insertar($tipo_doc, $numero_doc, $nombre, $apellido, $nacionalidad, $pueblo_indigena, $etnia);
            echo $rspta ? "Persona registrada" : "Persona NO pudo ser registra";
        } else { 
            $rspta = $evento->editar($idpersona, $tipo_doc, $numero_doc, $nombre, $apellido, $nacionalidad, $pueblo_indigena, $etnia);
            echo $rspta ? "Evento actualizado" : "Evento no se pudo actualizar";
        }
    break;
    
    case 'mostrar':
        $rspta = $evento->mostrar($idpersona);
